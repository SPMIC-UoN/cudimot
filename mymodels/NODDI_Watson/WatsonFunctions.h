...
{
  // Computes the spherical harmonic (SH) coefficients of the Watson's distribution 
  // with the concentration parameter k (kappa) and 12th order
  ...
  if (k > 30) {
    ...
    // Interpolate between exact and series expansion
    double ALL_EXACT = 0.45;
    double ALL_SERIES = 0.35;
    double exact_prop, series_prop;
    if (k > ALL_EXACT) {
      exact_prop = 1.0;
      series_prop = 0.0;
    }
    else if (k < ALL_SERIES) {
      exact_prop = 0.0;
      series_prop = 1.0;
    }
    else {
      exact_prop = (k - ALL_SERIES) / (ALL_EXACT - ALL_SERIES);
      series_prop = (ALL_EXACT - k) / (ALL_EXACT - ALL_SERIES);
    }

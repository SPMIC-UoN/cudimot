...
// SSFP !!! 
// In Ball & 3 Sticks model_1d the parameters P are:
// d   P[0]
// f1  P[1]
// th1 P[2]
// ph1 P[3]
...
// th2 P[5]
// ph2 P[6] 
// f3  P[7] - 3rd fibre added
// th3 P[8] - 3rd fibre added
// ph3 P[9] - 3rd fibre added
// S0  P[10]
...
MACRO T SSFP_Signal(T adc, T* P, T* CFP, T* FixP, T E1, T E2, T sa, T ca)
{
  ...
  T signal = sqrt_gpu(fabs_gpu(S0*Mminus_top/Mminus_bottom)*fabs_gpu(S0*Mminus_top/Mminus_bottom)+CFP[8]*CFP[8]); // updated P[8] -> P[10]
			 ...
			 T* FixP) 	// Fixed Parameters for each voxel
{
  ...
  T xv_3 = CFP[0]*sin_gpu(P[8])*cos_gpu(P[9])	// (bvec(1)*sinth3*cosph3
	 + CFP[1]*sin_gpu(P[8])*sin_gpu(P[9]) 	// + bvec(2)*sinth3*sinph3
	 + CFP[2]*cos_gpu(P[8]);		// + bvec(3)*costh2)- 3rd fibre added


  T adc_aniso1= P[0]*xv_1*xv_1;	// d*(pow(xv1,2))
  T adc_aniso2= P[0]*xv_2*xv_2;	// d*(pow(xv2,2))
  T adc_aniso3= P[0]*xv_3*xv_3;	// d*(pow(xv3,3))_ - 3rd fibre added
  ...
  T anisoterm_3=SSFP_Signal(adc_aniso3,P,CFP,FixP,E1,E2,sa,ca);//_ 3rd fibre


  T pred_signal= ((1-P[1]-P[4]-P[7])*isoterm + P[1]*anisoterm_1 + P[4]*anisoterm_2 + P[7]*anisoterm_3); // updated to include 3rd fibre and increment indices accordingly
...
MACRO bool ConstraintsMCMC(
		       ...
		       T* P) 	 // Estimated parameters
{
  if(P[1]<P[4] | P[4]<P[7] | (P[1]+P[4]+P[7])>1){       // updated to include 3rd fibre and increment indices accordingly
    //if f1<f2 reject 
    //if f1+f2>1 reject
    return false;
  }

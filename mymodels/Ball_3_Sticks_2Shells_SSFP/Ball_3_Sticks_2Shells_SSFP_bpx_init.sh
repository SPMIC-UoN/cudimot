...
mkdir -p ${subjdir}.${modelname}/Dtifit # make dtifit folder
part=0
...
############################################
########### following is for initialisation - added by SW; MKS
############################################
...
# mean_S0samples
for img in dyads1 dyads2 dyads3 mean_f1samples mean_f2samples mean_f3samples mean_S0samples; do
  if [ `${FSLDIR}/bin/imtest ${subjdir}.bedpostX/${img}` -eq 0 ]; then
  	echo "${subjdir}.bedpostX/${img} not found"
  	exit 1
  fi
done
...
fslmaths ${subjdir}.${modelname}/Dtifit/dtifit_MD -mul 2 ${InitializationDir}/d1.nii.gz
fslmaths ${subjdir}.${modelname}/Dtifit/dtifit_MD -mul 2 ${InitializationDir}/d2.nii.gz

# from spin-echo bedpostX
for img in dyads1 dyads2 dyads3 mean_f1samples mean_f2samples mean_f3samples mean_S0samples; do
  ${FSLDIR}/bin/imcp ${subjdir}.bedpostX/${img} $InitializationDir
done
...
echo ${InitializationDir}/d1 >> $InitializationFile # d1 P[0]
echo ${InitializationDir}/d2 >> $InitializationFile # d2 P[1]
echo ${InitializationDir}/mean_f1samples >> $InitializationFile # f1 P[2]
echo ${InitializationDir}/th1 >> $InitializationFile # th1 P[3]
echo ${InitializationDir}/ph1 >> $InitializationFile # ph1 P[4]
echo ${InitializationDir}/mean_f2samples >> $InitializationFile # f2 P[5]
echo ${InitializationDir}/th2 >> $InitializationFile # th2 P[6]
echo ${InitializationDir}/ph2 >> $InitializationFile # ph2 P[7]
echo ${InitializationDir}/mean_f3samples >> $InitializationFile # f3 P[8]
echo ${InitializationDir}/th3 >> $InitializationFile # th3 P[9]
echo ${InitializationDir}/ph3 >> $InitializationFile # ph3 P[10]
echo ${InitializationDir}/mean_S0samples >> $InitializationFile # S0 P[11]
...
opts=$opts" --data=${subjdir}/data --maskfile=$subjdir.${modelname}/nodif_brain_mask --partsdir=$partsdir --outputdir=$subjdir.${modelname} --forcedir --init_params=$InitializationFile" # added init params

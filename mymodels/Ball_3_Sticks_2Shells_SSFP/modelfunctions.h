...
// SSFP !!!
// In Ball & 3 Sticks model the parameters P are:
// d1  P[0]
// d2  P[1]
// f1  P[2]
// th1 P[3]
...
// th2 P[6]
// ph2 P[7]
// f3  P[8] - 3rd fibre added
// th3 P[9] - 3rd fibre added
// ph3 P[10] - 3rd fibre added
// S0  P[11]
...
// CFP[6]   diffGradDurs
// CFP[7]   flipAngle
// CFP[8]   whichshell  (0=first, 1=second)
...
MACRO T SSFP_Signal(T adc, T* P, T* CFP, T* FixP, T E1, T E2, T sa, T ca)
{
  ...
  T signal = sqrt_gpu(fabs_gpu(P[11]*Mminus_top/Mminus_bottom)*fabs_gpu(P[11]*Mminus_top/Mminus_bottom)+CFP[9]*CFP[9]); // updated P[8] -> P[11]
			 ...
			 T* FixP) 	// Fixed Parameters for each voxel
{
  ...
  T xv_1 = CFP[0]*sin_gpu(P[3])*cos_gpu(P[4])   // (bvec(1)*sinth1*cosph1
	...
	+ CFP[2]*cos_gpu(P[3]);			// + bvec(3)*costh1) - adjusted indices for inclusion of 3rd fibre

  T xv_2 = CFP[0]*sin_gpu(P[6])*cos_gpu(P[7])	// (bvec(1)*sinth2*cosph2
	 + CFP[1]*sin_gpu(P[6])*sin_gpu(P[7]) 	// + bvec(2)*sinth2*sinph2
	 + CFP[2]*cos_gpu(P[6]);		// + bvec(3)*costh2) - adjusted indices for inclusion of 3rd fibre

  T xv_3 = CFP[0]*sin_gpu(P[9])*cos_gpu(P[10])	// (bvec(1)*sinth3*cosph3
 	 + CFP[1]*sin_gpu(P[9])*sin_gpu(P[10]) 	// + bvec(2)*sinth3*sinph3
 	 + CFP[2]*cos_gpu(P[9]);		// + bvec(3)*costh3) - 3rd fibre added

  T adc_aniso1= adc_iso*xv_1*xv_1;	// d*(pow(xv1,2))
  T adc_aniso2= adc_iso*xv_2*xv_2;	// d*(pow(xv2,2))
  T adc_aniso3= adc_iso*xv_3*xv_3;	// d*(pow(xv3,2)) - 3rd fibre added
  ...
  T anisoterm_3=SSFP_Signal(adc_aniso3,P,CFP,FixP,E1,E2,sa,ca);

  T pred_signal= ((1-P[2]-P[5]-P[8])*isoterm + P[2]*anisoterm_1 + P[5]*anisoterm_2  + P[8]*anisoterm_3); // updated to include 3rd fibre and increment indices accordingly
...
MACRO bool ConstraintsMCMC(
		       ...
		       T* P) 	 // Estimated parameters
{
  if(P[2]<P[5] | P[5]<P[8] | (P[2]+P[5]+P[8])>1){ // updated to include 3rd fibre and increment indices accordingly
    //if f1<f2 reject
    //if f2>f3 reject
    //if f1+f2+f3>1 reject
    return false;
  }
  return true;
}
